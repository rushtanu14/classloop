...
3a. In draft Follow-up review, free-resource search sends only the teacher-entered topic to `/api/free-resources`, returns normalized Wikipedia/Open Library results, handles either provider failing, and adds a selected result only after the teacher clicks Add resource. Duplicate URLs stay disabled, malformed/provider-spoofed URLs stay hidden, and transcripts, rosters, notes, and student details never enter the request.
3b. The Filestack resource upload control remains locked until the teacher explicitly confirms the file is already intended for students. Only PDF/TXT/Markdown files up to 10 MB are accepted. The upload-session and finalization endpoints require a matching Supabase teacher session, rate-limit both IP and user, use a five-minute `pick`/`store` policy, bind the upload receipt and filename prefix to the teacher, and verify provider metadata server-side. Finalization must download through a 60-second private read policy, require a fresh authenticated ClamAV `clean` receipt, and create the long-lived share link only afterward. Malware, stale definitions, invalid receipts, timeouts, and scanner outages must return no share URL and attempt Filestack deletion. The UI may say no malware was detected at the recorded scan time, never “virus-free.” Never route transcript, roster, private-note, student-work, or demo-account files through Filestack. When any Filestack or scanner setting is absent, fail closed without exposing setup internals.
...
13. Paid/API-key/external-platform features do not pretend to be live: Whisper upload transcription only works through the hosted `/api/transcribe` endpoint when `OPENAI_API_KEY` is configured; Google Classroom posting, fully automatic Google Calendar/Docs actions, and any live Zoom bot remain OAuth/API setup work. Google Classroom and Zoom cloud surfaces are allowed only as honest scaffolds until OAuth/API credentials are connected. Recap email is separate: it may be live only when the server reports a configured Gmail app-password or SMTP sender.
...
- Composio Gmail is not an email path. Keep `CLASSLOOP_GMAIL_*` or `CLASSLOOP_SMTP_*` server-only and never replace them with `COMPOSIO_GMAIL_*` or `VITE_*` credentials.
- Connected-provider imports must use the authenticated read-only flow: teacher OAuth, bounded source list, explicit source choice, normalized field review, Continue to New Session, and a separate Apply selected fields action. Never expose raw provider JSON or provider ids, and never mutate the New Session form before Apply. Keep LMS posting, Composio Gmail, OpenAI transcription, custom transcription endpoints, live Zoom bots, and the removed in-person class capture mode out of the product. Browser-only online meeting capture is a Pro-only best-effort feature; transcript paste/upload remains the reliable manual path.
...
- Keyless public APIs are discovery inputs, not automatic trust. The current teacher-triggered resource search is limited to live-verified Wikipedia and Open Library endpoints, uses bounded queries, short timeouts, rate limiting, a short-lived server cache, strict response normalization, source links, and partial-failure fallback. Do not derive its query from private class records or add a provider solely because it appears in a directory.
- Filestack is an optional server-configured path only for teacher-approved shareable resources. Require authenticated teacher identity, provider Security enabled, short-lived signed upload scope, server metadata verification, bounded file type/size, explicit no-private-data acknowledgement, and the private self-hosted ClamAV gate. Fail closed on malware, stale definitions, invalid receipts, timeouts, or scanner outages; do not return a read link before a fresh clean receipt. Keep transcripts, rosters, notes, and student submissions on their existing local/review paths.
- Mailboxlayer stays disabled in the free build because its verified free tier does not include HTTPS. Do not transmit emails or API credentials over plaintext HTTP. Local syntax validation and Supabase inbox confirmation remain the account-validation path unless a future HTTPS plan and privacy-approved workflow are explicitly selected.
